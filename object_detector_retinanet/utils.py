...
def root_dir():
    if platform.system() == 'Linux':
        ...
        return os.path.join(os.getenv('HOME'), 'workspace', 'SKU110K_CVPR19')
...
def data_dir():
    if platform.system() == 'Linux':
        # return os.path.join(os.getenv('HOME'), 'Documents', 'SKU110K')
        return os.path.join(os.getenv('HOME'), 'workspace', 'Data', 'SKU110K')
    elif platform.system() == 'Windows':
        return os.path.abspath('C:/Users/{}/Documents/SKU110K/'.format(os.getenv('username')))


def image_path():
    return os.path.join(data_dir(), IMG_FOLDER)


def annotation_path():
    return os.path.join(data_dir(), ANNOTATION_FOLDER)
